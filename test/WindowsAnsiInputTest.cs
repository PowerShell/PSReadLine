    ...
    public partial class ReadLine
    {
            ...
            bool inputOnly = false)
        {
            for (var i = 0; i < keys.Length; i++)
            {
                ...
                if (i < keys.Length - 1) Assert.False(map.KeyAvailable);
            }
        ...
        public void ValidEscapeSequences()
        {
            ...
            CheckEscapeInput(map,
                new ConsoleKeyInfo('\0', ConsoleKey.DownArrow, true, false, false),
                StringToCKI("\x1b[1;2B")
            );

            // ^[[1;8C = Ctrl+Alt+Shift+RightArrow
            CheckEscapeInput(map,
                new ConsoleKeyInfo('\0', ConsoleKey.RightArrow, true, true, true),
                StringToCKI("\x1b[1;8C")
            );

            // ^[[1;6D = Ctrl+Shift+LeftArrow
            CheckEscapeInput(map,
                new ConsoleKeyInfo('\0', ConsoleKey.LeftArrow, true, false, true),
            ...
            // ^[[15;5~ = Control+F5
            CheckEscapeInput(map,
                new ConsoleKeyInfo('\0', ConsoleKey.F5, false, false, true),
        ...
        public void AltSequences()
        {
            ...
            // ^[[ = Alt+[
            CheckEscapeInput(map, default, StringToCKI("\x1b["), true);
            var processedKey = map.ReadKey();
            ...
            // ^[j = Alt+j
            CheckEscapeInput(map, default, StringToCKI("\x1bj"), true);
            processedKey = map.ReadKey();
            ...
            CheckEscapeInput(map, default, StringToCKI("\x1bX"), true);
            processedKey = map.ReadKey();
            ...
            // ^[^A = Alt+Ctrl+A
            CheckEscapeInput(map, default, StringToCKI("\x1b\x01"), true);
            processedKey = map.ReadKey();
