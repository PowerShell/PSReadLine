...
namespace Test
{
    ...
    internal class TestConsole : IConsole
    {
        ...
        protected TestConsole(int width, int height, bool mimicScrolling)
        {
            ...
            var bufferSize = mimicScrolling ? BufferWidth * 1000 : BufferWidth * BufferHeight;
        ...
        public virtual bool KeyAvailable =>
            index < inputOrValidateItems.Length && inputOrValidateItems[index] is ConsoleKeyInfo;
        ...
        {
            if (left != CursorLeft || top != CursorTop) _ignoreNextNewline = false;
        ...
        {
            var writePos = CursorTop * BufferWidth + CursorLeft;
            for (var i = 0; i < BufferWidth - CursorLeft; i++)
            {
        ...
        {
            for (var i = 0; i < buffer.Length; i++) buffer[i] = new CHAR_INFO(' ', ForegroundColor, BackgroundColor);
        }
        ...
        public override void SetCursorPosition(int left, int top)
        {
            if (left < 0 || left >= _bufferWidth)
                throw new ArgumentOutOfRangeException(nameof(left),
                    $"Value should be >= 0 and < BufferWidth({_bufferWidth}), but it's {left}.");

            if (top < 0 || top >= _bufferHeight)
                throw new ArgumentOutOfRangeException(nameof(top),
                    $"Value should be >= 0 and < BufferHeight({_bufferHeight}), but it's {top}.");
