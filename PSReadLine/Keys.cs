...
namespace Microsoft.PowerShell
{
    internal class PSKeyInfo : IEquatable<PSKeyInfo>
    {
        ...
        private static readonly ThreadLocal<StringBuilder> keyInfoStringBuilder = new(() => new StringBuilder());
        ...
        public char KeyChar
        {
            get
            {
                ...
                if (len == 1) return KeyStr[0];

                var last = KeyStr[len - 1];
                var nextToLast = KeyStr[len - 2];
                if (nextToLast == '+' && (last == '-' || last >= '0' && last <= '9')) return last;
        ...
        {
            return Equals(this, obj as PSKeyInfo);
        }
        ...
        internal static void TryGetCharFromConsoleKey(ConsoleKeyInfo key, ref char result, ref bool isDeadKey)
        {
            ...
            var scanCode = MapVirtualKey(virtualKey, 0x0 /*MAPVK_VK_TO_VSC*/);
        ...
        private static string KeyInfoAsString(ConsoleKeyInfo key)
        {
            ...
            if (char.IsControl(c))
            {
                ...
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                {
                    ...
                    isDeadKey = c == '\0' && consoleKey >= ConsoleKey.Oem1 && consoleKey <= ConsoleKey.Oem102 &&
                                !isCtrl;
    ...
    internal static class Keys
    {
        ...
        public static PSKeyInfo
            CtrlH = Ctrl('h'); // !Linux, generate (keychar: '\b', key: Backspace, mods: 0), same as CtrlBackspace
        ...
        public static PSKeyInfo
            CtrlM = Ctrl('m'); // !Linux, same as CtrlJ but 'showkey -a' shows they are different, CLR bug
