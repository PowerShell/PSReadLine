...
internal class WindowsAnsiCharMap : ICharMap
{
    ...
    private static readonly char[] _escOOrBracket1Chars =
    {
        'A', 'B', 'C', 'D', 'F', 'H', 'P', 'Q', 'R', 'S'
    };

    // Used with ^[[x sequences.
    private static readonly char[] _escBracketChars =
    {
        'A', 'B', 'C', 'D', 'F', 'H'
    };

    // ConsoleKeys matching ^[Ox, ^[[x, and ^[[1;nx.
    private static readonly ConsoleKey[] _escBracketConsoleKeys =
    {
        // A                B                     C                      D
    ...
    private static readonly ConsoleModifiers[] _escBracketModifiers =
    {
    ...
    private readonly Stopwatch _escTimeoutStopwatch = new();
    ...
    public WindowsAnsiCharMap(long escapeTimeout = 50)
    {
        ...
        _pendingKeys = new List<ConsoleKeyInfo>(6);
    ...
    public bool KeyAvailable
    {
        get
        {
            if (_readKeyIndexFrom < _readKeyIndexTo) return true;
    ...
    public bool InEscapeSequence => _pendingKeys[0].KeyChar == '\x1b' &&
                                    _escTimeoutStopwatch.ElapsedMilliseconds < EscapeTimeout;
    ...
    public void ProcessKey(ConsoleKeyInfo key)
    {
        ...
        if (_addKeyIndex > 1) ProcessMultipleKeys();
    }
    ...
    private void ProcessSingleKey(ConsoleKeyInfo key)
    {
        ...
        else
        {
            ...
            if (_addKeyIndex == 0) _readKeyIndexTo = 1;
        }
    ...
    private void ProcessControlKey(int i, char ch)
    {
        ...
        SetKey(i, new ConsoleKeyInfo(ch, consoleKey, shift, false, control));
        if (i == 0) _readKeyIndexTo = 1;
    }
