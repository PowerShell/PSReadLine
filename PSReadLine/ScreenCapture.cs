...
";
    ...
    private static extern bool WriteConsoleOutput(IntPtr consoleOutput, CHAR_INFO[] buffer, COORD bufferSize,
        COORD bufferCoord, ref SMALL_RECT writeRegion);

    [DllImport("KERNEL32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
    private static extern bool ReadConsoleOutput(IntPtr consoleOutput, [Out] CHAR_INFO[] buffer, COORD bufferSize,
        COORD bufferCoord, ref SMALL_RECT readRegion);
    ...
    private static void WriteBufferLines(CHAR_INFO[] buffer, int top, IConsole console)
    {
        ...
        var bufferLineCount = buffer.Length / bufferWidth;
    ...
    private static string GetColorTable(IConsole console)
    {
        ...
        if (GetConsoleScreenBufferInfoEx(handle, ref csbe))
            return GetRTFColorFromColorRef(csbe.Black) + GetRTFColorFromColorRef(csbe.DarkBlue) +
                   GetRTFColorFromColorRef(csbe.DarkGreen) + GetRTFColorFromColorRef(csbe.DarkCyan) +
                   GetRTFColorFromColorRef(csbe.DarkRed) + GetRTFColorFromColorRef(csbe.DarkMagenta) +
                   GetRTFColorFromColorRef(csbe.DarkYellow) + GetRTFColorFromColorRef(csbe.Gray) +
                   GetRTFColorFromColorRef(csbe.DarkGray) + GetRTFColorFromColorRef(csbe.Blue) +
                   GetRTFColorFromColorRef(csbe.Green) + GetRTFColorFromColorRef(csbe.Cyan) +
                   GetRTFColorFromColorRef(csbe.Red) + GetRTFColorFromColorRef(csbe.Magenta) +
                   GetRTFColorFromColorRef(csbe.Yellow) + GetRTFColorFromColorRef(csbe.White);
    ...
    {
        internal readonly ushort UnicodeChar;
        ...
        {
            if (!(obj is CHAR_INFO)) return false;

            var other = (CHAR_INFO) obj;
            return UnicodeChar == other.UnicodeChar && Attributes == other.Attributes;
        }
    ...
    private struct CONSOLE_SCREEN_BUFFER_INFO_EX
    {
        ...
        internal readonly SMALL_RECT srWindow;
        ...
        internal readonly COLORREF Black;
        internal readonly COLORREF DarkBlue;
        internal readonly COLORREF DarkGreen;
        internal readonly COLORREF DarkCyan;
        internal readonly COLORREF DarkRed;
        internal readonly COLORREF DarkMagenta;
        internal readonly COLORREF DarkYellow;
        internal readonly COLORREF Gray;
        internal readonly COLORREF DarkGray;
        internal readonly COLORREF Blue;
        internal readonly COLORREF Green;
        internal readonly COLORREF Cyan;
        internal readonly COLORREF Red;
        internal readonly COLORREF Magenta;
        internal readonly COLORREF Yellow;
        internal readonly COLORREF White;
    }
}
...
public partial class PSConsoleReadLine
{
    ...
    {
        var selectionTop = Renderer.Console.CursorTop;
        ...
        bool TooCloseToTop()
        {
            return currentY - console.WindowTop < margin;
        }

        bool TooCloseToBottom()
        {
            return console.WindowTop + console.WindowHeight - currentY < margin;
        }
        ...
        while (!done)
        {
            ...
            {
                if (TooCloseToTop()) Renderer.ScrollDisplayUpLine();
